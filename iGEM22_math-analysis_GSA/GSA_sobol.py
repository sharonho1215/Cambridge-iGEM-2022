...
for i in range(len(vals)):
    parameters = (
        ...
        vals[i][4],
        vals[i][5],)
    Y[i][:] = model_solve(antithetic_model, [0,0,0], 100000, 100000, parameters)[0][-5]
...
print('\n\n====z1 Sobol output====\n\n')
Si_z1 = sobol.analyze(problem, Y[:,1], print_to_console=True)
print('\n\n====z2 Sobol output====\n\n')
Si_z2 = sobol.analyze(problem, Y[:,2], print_to_console=True)
