...
mu_range = np.linspace(0.001,10,samples)
...
for sample in range(samples):
    for sample_2 in range(samples):
        ...
        mu_1 = mu_range[sample]
        mu_2 = mu_range[sample_2]
...
ax[0].set_xlim(x_i.min(), x_i.max())
ax[0].set_ylim(y_i.min(), y_i.max())
